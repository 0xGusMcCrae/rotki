...
AERODROME_SUGAR_CONTRACT = string_to_evm_address('0x2073D8035bB2b0F2e85aAF5a8732C6f397F9ff9b')  # Aerodrome Finance LP Sugar  # noqa: E501

class AerodromePoolData(NamedTuple):
    pool_address: ChecksumEvmAddress
    pool_name: str
    gauge_address: Optional[ChecksumEvmAddress]


def save_aerodrome_data_to_cache(
        write_cursor: DBCursor,
        database: 'DBHandler',
        new_data: list[AerodromePoolData],
) -> None:
    """
    Stores data about aerodrome pools and gauges in the global db cache tables.
    ...
    for pool in new_data:
        addresbook_entries = [AddressbookEntry(
            ...
            name=f'Aerodrome pool {pool.pool_name}',
        ...
        if pool.gauge_address is not None:
            addresbook_entries.append(AddressbookEntry(
                ...
                name=f'Gauge for Aerodrome pool {pool.pool_name}',
                blockchain=SupportedBlockchain.BASE,
            ))
        try:
            ...
            log.debug(
                f'Aerodrome address book names for pool {pool.pool_address} were not added. '
...
def read_aerodrome_pools_and_gauges_from_cache() -> tuple[set[ChecksumEvmAddress], set[ChecksumEvmAddress]]:  # noqa: E501
    ...
    with GlobalDBHandler().conn.read_ctx() as cursor:
        str_pool_addresses = globaldb_get_general_cache_values(cursor=cursor, key_parts=(CacheType.AERODROME_POOL_ADDRESS,))  # noqa: E501
        for pool_address in str_pool_addresses:
            pool_addresses.add(string_to_evm_address(pool_address))

        str_gauge_addresses = globaldb_get_general_cache_values(cursor=cursor, key_parts=(CacheType.AERODROME_GAUGE_ADDRESS,))  # noqa: E501
...
def query_aerodrome_data_from_chain_and_maybe_create_tokens(
        inquirer: 'BaseInquirer',
        existing_pools: list[ChecksumEvmAddress],
) -> Optional[list[AerodromePoolData]]:
    """
    Queries aerodrome data from chain from the Aerodrome Finance LP Sugar contract.
    If new pools are found their tokens are created and the pools are returned.
    (Find more details for the contract here: https://github.com/aerodrome-finance/sugar).

    May raise:
    - RemoteError if there is an error connecting with the remote source of data
    """
    data_contract = inquirer.contracts.contract(AERODROME_SUGAR_CONTRACT)
    pool_data = []
    offset = 0
    limit = 200
    ...
    for pool in pool_data:
        try:
            ...
            log.error(
                f'Skipping aerodrome pool {pool[0]}. Could not deserialize it while '
        ...
        try:
            ...
            log.error(
                f'Skipping aerodrome pool {pool[0]}. Could not deserialize an evm address while '
                f'decoding it. Additional information: {e}',
            )
            continue
        ...
        for token in (token0, token1, pool_address):  # create the tokens for the new pools. Keep in mind that the pool address is the address of the lp token received when depositing to the pool  # noqa: E501
            try:
                get_or_create_evm_token(
                    ...
                    seen=TokenSeenAt(description='Querying aerodrome pools'),
                    protocol=AERODROME_POOL_PROTOCOL if token == pool_address else None,  # mark the lp tokens with the protocol to identify them for special treatment for price calculation  # noqa: E501
                )
            except NotERC20Conformant as e:
                log.error(
                    f'Skipping aerodrome token {token} because it is not a valid ERC20 token. {e}',
                )
                break
        else:
            pools.append(
...
def query_aerodrome_data(inquirer: 'BaseInquirer') -> Optional[list[AerodromePoolData]]:
    """
    Queries aerodrome pools and tokens.
    Returns a list of pool data if the query was successful.
    """
    with GlobalDBHandler().conn.read_ctx() as cursor:
        existing_pools = [
            string_to_evm_address(address)
            for address in globaldb_get_general_cache_like(cursor=cursor, key_parts=(CacheType.AERODROME_POOL_ADDRESS,))  # noqa: E501
        ]
    try:
        pools_data = query_aerodrome_data_from_chain_and_maybe_create_tokens(
            inquirer=inquirer,
            existing_pools=existing_pools,
        )
    except RemoteError as err:
        log.error(f'Could not query chain for aerodrome pools due to: {err}')
        pools_data = None
